...
from app.utils.principles import MAS_PRINCIPLES
from app.utils.security import RequireAgentToken
from app.agents.autogen_manager import AgentManager
router = APIRouter()
@router.get("/agent/principles")
async def get_principles():
    return JSONResponse(MAS_PRINCIPLES)
...
async def agent_message(request: Request):
    try:
        ...
        if str(data.get("engine", "")).lower() == "autogen":
            ...
            logger.info("agent_message: using AutoGen orchestration")
...
async def edit_agent(request: Request, _sec: None = RequireAgentToken):
    try:
        data = await request.json()
...
async def publish_existing_agent(request: Request, _sec: None = RequireAgentToken):
    try:
        data = await request.json()
...
# async def plan_and_run_manager(request: Request, _sec: None = RequireAgentToken):
#     data = await request.json()
...
async def plan_and_run_manager(request: Request, _sec: None = RequireAgentToken):
    ...
    # FIX: ensure all dates, decimals, UUIDs etc. become JSON-safe
...
async def multi_agent_chat(request: Request, _sec: None = RequireAgentToken):
